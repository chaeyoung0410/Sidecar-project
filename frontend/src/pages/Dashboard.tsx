    ...
    <div className="md:grid md:grid-cols-[220px_minmax(0,1fr)] lg:grid-cols-[236px_minmax(0,1fr)]">
      ...
      <div className="min-w-0 pb-24 md:pb-0">
        <div className="mx-auto max-w-[1180px] px-5 pb-12 pt-[max(1.25rem,env(safe-area-inset-top))] sm:px-8 lg:px-10">
          ...
          {route === '#git' && <GitStatusPanel status={workspace.gitStatus} loading={workspace.loading} error={workspace.gitError} hasProject={workspace.currentProject !== null} onRefresh={() => void workspace.refresh()} onManageProjects={() => setProjectsOpen(true)} onCommit={workspace.commitChanges} onSuggestCommitMessages={workspace.suggestCommitMessages} onGetPushPreview={workspace.getPushPreview} onPush={workspace.pushChanges} onGetPullPreview={workspace.getPullPreview} onPull={workspace.pullChanges} />}
