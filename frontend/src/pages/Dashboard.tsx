...
export function Dashboard() {
  ...
  return <main className="min-h-screen bg-ink text-[#f5f5f7]">
    <div className="md:grid md:grid-cols-[220px_minmax(0,1fr)] lg:grid-cols-[236px_minmax(0,1fr)]">
      <AppSidebar active={sidebarRoute} onNavigate={navigate} onManageActions={() => { setActionToEdit(null); navigate('#actions'); setActionsOpen(true) }} />
      <div className="min-w-0 pb-24 md:pb-0">
        <div className="mx-auto max-w-[1180px] px-5 pb-12 pt-[max(1.25rem,env(safe-area-inset-top))] sm:px-8 lg:px-10">
          ...
          {route === '#git' && <GitStatusPanel status={workspace.gitStatus} loading={workspace.loading} error={workspace.gitError} remoteStatus={workspace.remoteStatus} remoteLoading={workspace.remoteLoading} remoteError={workspace.remoteError} hasProject={workspace.currentProject !== null} onRefresh={() => void workspace.refresh()} onRefreshRemote={workspace.refreshRemoteStatus} onManageProjects={() => setProjectsOpen(true)} onCommit={workspace.commitChanges} onSuggestCommitMessages={workspace.suggestCommitMessages} onGetPushPreview={workspace.getPushPreview} onPush={workspace.pushChanges} onGetPullPreview={workspace.getPullPreview} onPull={workspace.pullChanges} />}
    ...
    <ActionManager open={actionsOpen} initialAction={actionToEdit} actions={actionState.actions} commands={commandState.commands} onClose={() => { setActionsOpen(false); setActionToEdit(null) }} onCreate={actionState.createAction} onUpdate={actionState.updateAction} onDelete={actionState.deleteAction} onMove={actionState.moveAction} />
