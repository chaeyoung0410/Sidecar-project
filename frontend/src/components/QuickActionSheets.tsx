...
function GitPushSheet({ gitStatus, onClose, onOpenFull, onPreview, onPush }: { gitStatus: GitStatus | null; onClose: () => void; onOpenFull: () => void; onPreview: QuickActionSheetsProps['onGetPushPreview']; onPush: QuickActionSheetsProps['onPush'] }) {
  ...
  const [error, setError] = useState<string | null>(null)
  const [success, setSuccess] = useState(false)
  useEffect(() => { void onPreview().then(setPreview).catch((requestError) => setError(requestError instanceof Error ? requestError.message : 'Git Push 정보를 불러오지 못했습니다.')).finally(() => setBusy(false)) }, [onPreview])
  ...
  return <QuickActionSheet title="Git Push" subtitle="현재 Branch의 Commit을 origin으로 Push합니다." size="sm" status={requestStatus(busy, success, error)} autoCloseMs={success ? 850 : undefined} onClose={onClose}>{success ? <SuccessMessage>Git Push가 완료되었습니다.</SuccessMessage> : <div className="space-y-4">{busy && !preview ? <p className="py-6 text-center text-sm text-zinc-500">Push 상태를 확인하고 있습니다…</p> : preview ? <dl className="space-y-3 rounded-2xl bg-black/20 p-4 text-sm"><div className="flex justify-between"><dt className="text-zinc-500">현재 Branch</dt><dd className="font-mono text-zinc-200">{preview.branch}</dd></div><div className="flex justify-between"><dt className="text-zinc-500">Push할 Commit</dt><dd className="text-apple">{preview.ahead}개</dd></div><div className="flex justify-between"><dt className="text-zinc-500">대상</dt><dd className="font-mono text-zinc-200">origin/{preview.branch}</dd></div></dl> : !gitStatus && <EmptyGit onOpenFull={onOpenFull} />}<ErrorMessage message={error} /><div className="flex items-center justify-between"><button type="button" onClick={onOpenFull} className="text-sm text-zinc-500 hover:text-apple">Git 전체 보기 →</button><button type="button" disabled={busy || !preview?.ahead} onClick={() => void push()} className="rounded-xl bg-apple px-5 py-3 text-sm font-semibold text-white disabled:opacity-30">Git Push</button></div></div>}</QuickActionSheet>
}
...
function GitPullSheet({ onClose, onOpenFull, onPreview, onPull }: { onClose: () => void; onOpenFull: () => void; onPreview: QuickActionSheetsProps['onGetPullPreview']; onPull: QuickActionSheetsProps['onPull'] }) {
  ...
  useEffect(() => { void onPreview().then(setPreview).catch((requestError) => setError(requestError instanceof Error ? requestError.message : 'Git Pull 정보를 불러오지 못했습니다.')).finally(() => setBusy(false)) }, [onPreview])
  const pull = async () => { setBusy(true); setError(null); try { const next = await onPull(); setResult(next); if (!next.success && !next.conflict) setError(next.message) } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Git Pull에 실패했습니다.') } finally { setBusy(false) } }
  return <QuickActionSheet title="Git Pull" subtitle="원격 Repository의 최신 변경사항을 가져옵니다." size="sm" status={requestStatus(busy, success, error || (result?.conflict ? 'conflict' : null))} autoCloseMs={success ? 900 : undefined} onClose={onClose}>{success ? <SuccessMessage>{result?.already_up_to_date ? '이미 최신 상태입니다.' : 'Git Pull이 완료되었습니다.'}</SuccessMessage> : <div className="space-y-4">{busy && !preview ? <p className="py-6 text-center text-sm text-zinc-500">로컬 변경사항을 확인하고 있습니다…</p> : preview && <dl className="space-y-3 rounded-2xl bg-black/20 p-4 text-sm"><div className="flex justify-between"><dt className="text-zinc-500">현재 Branch</dt><dd className="font-mono text-zinc-200">{preview.branch}</dd></div><div className="flex justify-between"><dt className="text-zinc-500">로컬 변경 파일</dt><dd className={preview.changed_files.length ? 'text-[#ffd60a]' : 'text-zinc-200'}>{preview.changed_files.length}개</dd></div>{preview.changed_files.length > 0 && <p className="border-t border-white/[0.08] pt-3 text-xs leading-5 text-[#ffd60a]">Merge Conflict가 발생할 수 있습니다. 변경사항은 자동 Stash하지 않습니다.</p>}</dl>}{result?.conflict && <div className="rounded-2xl border border-rose-400/20 bg-rose-400/[0.05] p-4"><p className="font-semibold text-rose-300">Merge Conflict</p><p className="mt-2 text-sm text-zinc-400">Mac의 VS Code에서 다음 파일을 확인해주세요.</p><div className="mt-3 space-y-1 font-mono text-xs text-zinc-300">{result.conflict_files.map((file) => <p key={file}>{file}</p>)}</div></div>}<ErrorMessage message={error} /><div className="flex items-center justify-between"><button type="button" onClick={onOpenFull} className="text-sm text-zinc-500 hover:text-apple">Git 상세 보기 →</button>{!result?.conflict && <button type="button" disabled={busy || !preview} onClick={() => void pull()} className="rounded-xl bg-apple px-5 py-3 text-sm font-semibold text-white disabled:opacity-30">{preview?.changed_files.length ? '그래도 Git Pull' : 'Git Pull'}</button>}</div></div>}</QuickActionSheet>
}
