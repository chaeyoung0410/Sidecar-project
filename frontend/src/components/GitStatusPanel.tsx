...
export function GitStatusPanel({ status, loading, error, hasProject, onRefresh, onManageProjects, onCommit, onGetPushPreview, onPush }: GitStatusPanelProps) {
    ...
    <section id="git" aria-labelledby="repository-title" className="py-12 sm:py-16">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          {hasProject && status && (
            <button type="button" disabled={busy || loading} onClick={() => void openPushConfirmation()} className="rounded-xl bg-[#2c2c2e] px-4 text-sm font-semibold text-white hover:bg-[#3a3a3c] disabled:opacity-40">Git Push</button>
          )}
